package com.example.sofra.ui.fragments.restaurant.orders;
 ...
 */
public class CurrentRestaurantOrderFragment extends Fragment {


    @BindView(R.id.Current_RestaurantOrder_Rv)
    RecyclerView CurrentRestaurantOrderRv;
    Unbinder unbinder;
    ...
    public CurrentRestaurantOrderFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_current_restaurant_order, container, false);
    ...
    private void getcurrent() {
        apiServices.restaurantMyOrders(api_token_restaurant, "current", 1).enqueue(new Callback<RestaurantOrder>() {
            @Override
            public void onResponse(Call<RestaurantOrder> call, Response<RestaurantOrder> response) {
                String msg = response.body().getMsg();
                Long status = response.body().getStatus();
                if (status == 1) {
                    adapterRestaurantOrder = new AdapterRestaurantOrder(getActivity(), getContext(), listOfOrder, 2);
                    Toast.makeText(getActivity(), msg, Toast.LENGTH_SHORT).show();
                    List<Datum> data = response.body().getData().getData();
                    CurrentRestaurantOrderRv.setLayoutManager(new LinearLayoutManager(getActivity()));
                    CurrentRestaurantOrderRv.setAdapter(adapterRestaurantOrder);
